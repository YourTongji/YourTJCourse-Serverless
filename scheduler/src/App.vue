<template>
  <a-config-provider :locale="locale">
    ...
    v-model:open="openOverview"
      ...
      :centered="true"
      :maskClosable="true"
      :bodyStyle="courseModalBodyStyle"
...
function normalizeDisplayList(value: unknown): string[] {
  if (typeof value === 'string') {
    ...
    if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
      try {
        return normalizeDisplayList(JSON.parse(trimmed));
      } catch {
        return [trimmed];
      }
    }
  ...
  if (Array.isArray(value)) {
    return value.flatMap((item) => normalizeDisplayList(item)).filter(Boolean);
  }
  return [];
}
...
export default {
  ...
  computed: {
    ...
    courseModalStyle() {
      return this.isMobile
        ? { width: 'calc(100vw - 24px)', maxWidth: 'none' }
        : { width: '80%', maxWidth: 'none' };
    },
    courseModalBodyStyle() {
      return this.isMobile
  ...
  methods: {
    normalizeDetailCourse(detail: Record<string, unknown>) {
      ...
      return {
        ...detail,
        campus: normalizeDisplayList(detail?.campus).join('、'),
        status: 0,
        arrangementInfo,
      };
    },
    ...
    async stageCourses() {
      ...
      for (const key of compulsoryCourses) {
        ...
        const _courseObject = {
          ...
          courseType: "必",
          courseNature: normalizeDisplayList((originalCourse as any).courseNature),
          teacher: [],
          status: 0,
          ...
          for (const courseCode of optionalCodes) {
            try {
              ...
              const _courseObject = {
                ...
                courseNature: normalizeDisplayList((_roughCourse as any).courseNature || (_roughCourse as any).courseLabelName),
                teacher: [],
                status: 0,
