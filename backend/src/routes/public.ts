...
async function loadReviewDislikedSetByClient(
  db: D1Database,
  reviewIds: number[],
  clientId: string
) {
  ...
  try {
    ...
    try {
      ...
      if (cached) {
        ...
        const dislikedSet = await loadReviewDislikedSetByClient(c.env.DB, reviewIds, clientId)
    ...
    if (teacherName) {
      ...
      if (teacherIds.length > 0) {
        ...
        const rows = await c.env.DB
          ...
          .bind((course as any).code, (course as any).name, ...teacherIds)
          .all<{ id: number }>()
      ...
      const rows = await c.env.DB
        ...
        .bind((course as any).code, (course as any).name, (course as any).teacher_id ?? -1)
        .all<{ id: number }>()
      for (const r of rows.results || []) matchedIds.add(Number((r as any).id))
    }
...
publicRoutes.get('/course/by-code/:code', async (c) => {
  try {
    ...
    if (clientId && (reviewsWithSqid || []).length > 0) {
      ...
      dislikedSet = await loadReviewDislikedSetByClient(c.env.DB, ids, clientId)
    }
...
publicRoutes.get('/review/by-wallet/:userHash', async (c) => {
  try {
    ...
    const reviews = addSqidToReviews(results.slice(0, limit)).map((review: any) => ({
      ...
      dislike_count: Number(review?.disapprove_count || 0),
      disliked: false
    }))
...
publicRoutes.post('/review/:id/report', async (c) => {
  ...
  if (hasDescription && typeof body.description !== 'string') {
    return c.json({ error: '举报说明格式无效' }, 400)
  }
  const description = hasDescription ? normalizeReportDescription(body.description) : ''
    ...
    await c.env.DB
      .prepare(
        `UPDATE review_reports
         SET reason = ?, description = ?, status = 'open', resolved_at = NULL, updated_at = strftime('%s', 'now')
  ...
  if ((isNewReport || isReopenedReport) && effectiveReportId) {
    ...
    if (debugFeishu) {
      ...
      // Node 环境无 executionCtx：不阻塞响应，fire-and-forget 后台发送
      feishuPromise.catch(() => {})
    }
  }
  ...
  return {
    ...
    <style>
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      h1{font-size:22px;font-weight:800;margin-bottom:12px;line-height:1.3;overflow-wrap:break-word}
      .msg{font-size:15px;line-height:1.6;color:#475569;margin-bottom:24px;overflow-wrap:break-word}
      ...
      .btn:focus-visible{outline:3px solid rgba(14,165,233,.6);outline-offset:2px}
