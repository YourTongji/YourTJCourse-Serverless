...
```json
{
  ...
  "description": "请说明举报原因和具体的违规内容，例如指出评价中哪部分内容违反社区规范。"
}
```

- `description` 由前端强制填写，提交前要求说明包含举报原因和具体的违规内容，不少于 50 字且禁止粘贴。
- 后端兼容旧客户端未传 `description` 的请求；但一旦传入该字段，必须为字符串、说明正文不少于 50 字且最多 1000 字，并会展示在飞书举报通知卡片中。
...
同一服务端派生客户端对同一评价重复举报会更新原因、说明与时间，不会重复创建多条记录；若该举报此前已被处理，重复举报会将其重新打开并再次通知管理员。
